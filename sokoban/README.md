...
- 🧩 **内置关卡**：10 个由易到难的官方关卡，发布前已用 BFS 求解器校验全部可解
- ↩️ **撤销 / 重置**：逐步撤销与一键重置当前关卡
- 🔢 **关卡导航**：上一关 / 下一关自由切换；最佳步数按棋谱（地图指纹）记录，关卡重排序也不漂移
- 💾 **进度保存**：进度与最佳步数保存于 LocalStorage
- 🔀 **合并关卡池**：所有入口（开始游戏 / 悬浮面板 / 试玩）统一加载「官方 + 自定义」合并关卡，顺序切换不遗漏
- 🌓 **主题切换**：明亮 / 暗黑双主题，与全站一致
...
- 👀 **查看关卡**：官方 / 自制关卡简图卡片，支持试玩、分享、导出 / 批量导出、删除，并标注「无解」警告
- 🔍 **关卡搜索**：按关卡名称实时过滤「默认 / 我的」两个区段
- 🔗 **分享与导入**：一键复制分享链接（base64 编码），好友打开即自动导入
- 📤 **关卡导出**：每张卡片可导出为 JSON（含名称与地图）或 TXT（纯字符画）
- 📦 **批量导出**：查看关卡页「我的关卡」可一键导出全部，或按搜索结果导出命中的全部关卡（导出文件名自动拼接搜索词）为单个 JSON 包
...
| 查看关卡 | `levels.html` | 简图卡片：试玩 / 分享 / 导出 / 批量导出 / 删除，上传导入、搜索 |
...
│   └── levels-manifest.json# 关卡清单（驱动加载、签名校验自动刷新）
├── css/
│   ├── home.css            # 入口页样式
│   ├── style.css           # 游戏页样式（含明暗主题变量、SVG mask 图标）
│   ├── levels.css          # 查看关卡页样式
│   ├── levelpicker.css     # 选择关卡悬浮面板样式
│   └── editor.css          # 制作关卡页样式
├── js/
│   ├── game.js             # 游戏核心（移动 / 推箱 / 撤销 / 胜负 / 控制栏 / 合并关卡池）
│   ├── levelpicker.js      # 选择关卡悬浮面板（官方+自定义合并、当前关高亮、就地切换）
│   ├── levels.js           # 关卡数据加载与解析（JSON / 字符画）
│   ├── solver.js           # BFS 可解性求解器
│   ├── map.js              # 共享简图渲染（mini-map）
│   ├── storage.js          # 本地存储（最佳步数按棋谱指纹 / 进度）
│   ├── userlevels.js       # 用户关卡 CRUD + base64 分享码 + 导出 / 批量导出
│   ├── viewer.js           # 查看关卡页逻辑（卡片 / 搜索 / 缓存 / 批量导出）
